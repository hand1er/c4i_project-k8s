...
for i in range(HASH_NUM):
    ...
    addr_data.append("10." + "96." + number_extraction[1] + number_extraction[2]  + "." + number_extraction[4] + number_extraction[5])
    f.write(addr_data[i] + "\n")
f.close()
...
for i in range(HASH_NUM):
    ...
    f.write(service_template.replace('keystone-n', 'keystone-' + str(i+1)).replace('ip_addr', addr_data[i]))
